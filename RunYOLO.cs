...
public class RunYOLO : MonoBehaviour
{
    ...
    public void ExecuteML()
    {
        ...
        if (video && video.texture)
        {
            ...
            if (video.width > imageWidth)
            {
                offset_x = (video.width - imageWidth) / 2f;
                offset_x /= video.width; // convert to 0~1 range
            }
            else
            {
                offset_x = 0;
            }
            if (video.height > imageHeight)
            {
                offset_y = (video.height - imageHeight) / 2f;
                offset_y /= video.height; // convert to 0~1 range
            }
            else
            {
                offset_y = 0;
            ...
            Graphics.Blit(video.texture, targetRT, new Vector2(1f / aspect, 1), new Vector2(offset_x, 0));
